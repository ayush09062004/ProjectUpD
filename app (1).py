...
def perform_search(search_query: str, serper_key: str, max_results: int = 10) -> list:
    """Search using Serper (Google Search API) — reliable on cloud, no IP blocking."""
    results = []
    try:
        resp = requests.post(
...
**Problem Summary:** <1 sentence describing the citizen's problem>
...
**Reasoning:** <2-3 sentences explaining why this body is responsible under Indian law/governance>
**Suggested Action:** <one concrete step with portal name or helpline number e.g. File complaint on pgportal.gov.in or call 1533>
**Official Website:** <one verified, working Indian government URL — only use well-known portals like pgportal.gov.in, swachhbharatmission.gov.in, mohua.gov.in, jansunwai.up.nic.in, etc. Do NOT invent URLs.>
**Sources Used:** <comma-separated list of source titles from the Retrieved Context that you actually used. If none were useful, write "Built-in knowledge">"""
...
Retrieved Context (each entry labelled with its source title):
{context or 'No web context retrieved.'}

Instructions:
- Identify the responsible Indian government authority.
- Ignore any non-Indian sources.
...
with col_form:
    ...
    serper_key = st.text_input("Serper API Key (free at serper.dev)", type="password", placeholder="serper_...")
    st.caption("Serper gives 2,500 free Google searches/month — no card needed.")
...
with col_result:
    ...
    if submit:
        ...
        if not serper_key:
            errors.append("Please enter your Serper API Key (free at serper.dev).")
        ...
        if errors:
            ...
            try:
                ...
                # ── Official URL: only show well-known gov portals, skip if unknown ──
                ...
                # ── Sources the LLM cited ──
                sources_used = clean(parsed.get("Sources Used", ""))
                if sources_used and sources_used.lower() != "built-in knowledge":
                    st.markdown(f"**📚 Sources Used by LLM:** {sources_used}")

                # ── Full pipeline reasoning trace ──
                with st.expander("🧩 How this answer was generated"):
                    retrieved_list = "\n".join(
                        f"- **{r.get('title','Untitled')}**\n  {r.get('href','')}\n  _{r.get('body','')[:120]}..._"
                        for r in filtered[:5]
                    ) or "⚠️ No sources retrieved from web search."

                    st.markdown(f"""
#### Step 1 — Jurisdiction Prediction
Problem classified as **{jurisdiction} Government** responsibility using model `openai/gpt-oss-20b`.

#### Step 2 — Web Search (Serper / Google)
Query: `{search_q}`
Results found: **{len(raw_results)}** (after filtering blocked domains)

#### Step 3 — Retrieved Sources
{retrieved_list}

#### Step 4 — Context Extraction
{"✅ " + str(len(context)) + " characters extracted from top sources via trafilatura." if context else "⚠️ No content extracted — LLM relied on built-in knowledge of Indian governance."}

#### Step 5 — LLM Final Reasoning
Model `openai/gpt-oss-120b` was given the problem, location, predicted jurisdiction, and retrieved context.
It identified the responsible authority, department, reasoning, suggested action, and official portal.

**Sources the LLM cited:** {sources_used or "Built-in knowledge"}
""")
